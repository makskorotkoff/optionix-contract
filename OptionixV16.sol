    ...
    UUPSUpgradeable
{
    ...
    function betResult(
        ...
        uint _expGasCost = _expGasUsed * tx.gasprice;
        require(_amount >= _expGasCost, "Gas cost too high");
        if (comissionsPool < 0) {
            require(
        ...
        if (_result == 1) {
            ...
            if (_amount < _comissionValue + _expGasCost) {
                _comissionValue = _amount - _expGasCost;
            }
            _numericResult = _amount - _comissionValue - _expGasCost;

            userBalance[_better] += _numericResult;
            comissionsPool = comissionsPool - int(_amount - _comissionValue);
        } else {
            userBalance[_better] -= _amount;
            comissionsPool = comissionsPool + int(_amount - _expGasCost);
        }
        ownerComissions += _expGasCost;
